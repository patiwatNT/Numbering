...
export class ReportN16DetailComponent implements OnInit{
    ...
    if(this.block.includes('1000')){
      this.backendService.findReportN16Block1000().subscribe(
        (response)=>{
          ...
          this.excelObj = {block:this.block,blockAmount:this.blockAmount,reportN16DetailList:this.reportN16DetailList};
  ...
  createTextFile(list:any[]):string{
    let data = 'Block Range|หมายเลขเริ่มต้น|หมายเลขสิ้นสุด|พื้นที่|จำนวนเลขหมาย|ค่าธรรมเนียมรายเดือน (บาท)\n';
    for (let i of list) {
        data += `${i.blockRange}|${i.startTel}|${i.endTel}|${i.location}|${i.numberAmount}|${i.annualFee}\n`;
    }
    return data;
  }
}
